...
namespace ExpressBI.SubForm
{
    public partial class DialogSelectApmas : Form
    {
        ...
        private void DialogSelectApmas_Load(object sender, EventArgs e)
        {
            ...
            {
                var selected_row = this.dataGridView1.Rows.Cast<DataGridViewRow>().Where(r => ((string)r.Cells[this.col_supcod.Name].Value).CompareTo(this.init_supcod) >= 0).FirstOrDefault();
